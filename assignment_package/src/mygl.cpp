...
MyGL::MyGL(QWidget *parent)
    : OpenGLContext(parent),
      ...
      m_mousePosPrev(),
    m_vertDisplay(this), m_edgeDisplay(this), m_faceDisplay(this), m_geomMesh(this)
{
...
void MyGL::createList() {
    for (auto& v: m_geomMesh.vertices) {
        emit sig_sendVertex(v.get());
    }

    for (auto& he: m_geomMesh.halfEdges) {
        emit sig_sendHalfEdge(he.get());
    }

    for (auto& f: m_geomMesh.faces) {
        emit sig_sendFace(f.get());
    }
}

void MyGL::generateMesh(QString filename) {
    meshLoaded = true;
...
void MyGL::initializeGL()
{
    ...
    m_geomMesh.initializeAndBufferGeometryData();
}
...
void MyGL::keyPressEvent(QKeyEvent *e) {
    ...
    case Qt::Key_N :
        ...
        m_edgeDisplay.initializeAndBufferGeometryData();
        update();
        break;

    case Qt::Key_M :
        m_edgeDisplay.updateEdge(m_edgeDisplay.representedEdge->symPtr);
        m_edgeDisplay.initializeAndBufferGeometryData();
        update();
        break;
    ...
    case Qt::Key_F :
        ...
        m_faceDisplay.initializeAndBufferGeometryData();
        update();
        break;

    case Qt::Key_V :
        m_vertDisplay.updateVertex(m_edgeDisplay.representedEdge->vert);
        m_vertDisplay.initializeAndBufferGeometryData();
        update();
        break;

    case Qt::Key_H :
        if(e->modifiers() & Qt::ShiftModifier) {
            m_edgeDisplay.updateEdge(m_faceDisplay.representedFace->halfEdge);
            m_edgeDisplay.initializeAndBufferGeometryData();
            update();
        } else {
            m_edgeDisplay.updateEdge(m_vertDisplay.representedVertex->halfEdge);
            m_edgeDisplay.initializeAndBufferGeometryData();
            update();
        }
        break;
